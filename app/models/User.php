...
class User extends Model
{
     ...
     * @return array|bool 取得に成功した場合、ユーザ情報を返却。取得失敗の場合はfalseを返却
     */
    public function search(array $attributes, int $offset = 0, int $limit = 10): array|bool
    {
        $andWheres = [];
    ...
    public function authenticate(string $email, string $password): array|bool
    {
        ...
        $user = $this->search($searchParam, 0, 1);
     ...
     * @return bool 登録成否
     */
    public function store(string $email, string $name, string $password): bool
    {
        $parameters = [
            'email' => $email,
            'name' => $name,
     ...
     * @return bool 更新成否
     */
    public function update(int $id, string $email, string $name, string $password = ''): bool
    {
        $parameters = [
            'email' => $email,
            'name' => $name,
